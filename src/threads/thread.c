                    ...
                    thread_func *function, void *aux)
{
    ...
    sema_init(&child->sema_load, 1);
    sema_init(&child->sema_leave, 1);
...
/* Frees all children threads of the current thread. */
void free_children_threads()
{
...
struct child_thread *get_child_thread(struct thread *parent, tid_t tid)
{
    struct list_elem *e = get_child_elem(parent, tid);
    if (e == NULL)
    {
        return NULL;
    }
...
/* Returns the child thread elem with the given TID. */
struct list_elem *get_child_elem(struct thread *parent, tid_t tid)
{
    struct child_thread *child;
    struct list_elem *e;
