...
tid_t process_execute(const char *file_name)
{
    ...
    char file_name_copy[length];
    strlcpy(file_name_copy, file_name, length);
    char *save_ptr;
    char *parsed_file_name = strtok_r(file_name_copy, " ", &save_ptr);
...
static void setup_args(char *save_ptr, void **esp, char *file_name) // double check this
{
    ...
    *esp -= 4;
    ...
    {
        int length = strlen(file_name_array[i]) + 1;
        *esp -= length;
        memcpy(*esp, file_name_array[i], length);
        file_name_array[i] = *esp;
    }
    ...
    *esp -= 4;
    ...
    {
        *esp -= sizeof(char *);
        memcpy(*esp, &file_name_array[i], sizeof(char *));
    }

    /* Push argv */
    void *argv_ptr = *esp;
    *esp -= sizeof(void *);
    memcpy(*esp, &argv_ptr, sizeof(void *));

    /* Push argc */
    *esp -= 4;
    memset(*esp, argc, sizeof(char)); // could change?

    /* Push fake return address */
    *esp -= 4;
    memset(*esp, 0, 4);

    /* Deal with stack overflows */
...
start_process(void *file_name_)
{
    ...
    /* Parse the file name */
    int length = strlen(file_name_) + 1;
    char file_name_copy[length];
    strlcpy(file_name_copy, file_name_, length);
    char *save_ptr;
...
int process_wait(tid_t child_tid UNUSED)
{
    ...
    struct list_elem *child_elem = get_child_elem(thread_current(), child_tid);
    if (child_elem != NULL)
    {
        struct child_thread *child = list_entry(child_elem, struct child_thread, elem);
        sema_down(&child->sema_leave);
...
{
    /* Get rid of memory blocks, etc. */
    struct thread *cur = thread_current();
    enum intr_level old_level = intr_disable();
    free_all_files();
    free_children_threads();
    intr_set_level(old_level);
    free(cur->pcb);

    struct child_thread *child;
    child = get_child_thread(cur->parent, cur->tid);
    printf("%s: exit(%d)\n", cur->name, child->exit_status);
    sema_up(&child->sema_leave);
    uint32_t *pd;
...
bool load(const char *file_name, void (**eip)(void), void **esp)
{
    ...
    /* Setting file in PCB, + disabling changes from anywhere else */
    file_deny_write(file);
#ifdef USERPROG
    t->pcb->file = file; // could change?
#endif
...
{
    struct thread *cur = thread_current();
    struct list_elem *e = list_begin(&cur->pcb->fd_table);
    struct file_descriptor *fd_entry;
    enum intr_level old_level = intr_disable();
    while (e != list_end(&cur->pcb->fd_table))
    {
...
struct list_elem *get_file_elem_by_fd(int fd)
{
    ...
    struct file_descriptor *fd_entry;
    struct list_elem *e;
    for (e = list_begin(&cur->pcb->fd_table); e != list_end(&cur->pcb->fd_table); e = list_next(e))
    {
        fd_entry = list_entry(e, struct file_descriptor, elem);
        if (fd_entry->fd == fd)
        {
